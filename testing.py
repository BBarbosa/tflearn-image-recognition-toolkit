...
parser = argparse.ArgumentParser(
    description="Testing script for CNNs.",
...
parser.add_argument(
    "--model",
    required=True,
    help="<REQUIRED> run identifier (id) / model's path",
    type=str)
# optional arguments
parser.add_argument(
    "--test_dir",
...
parser.add_argument(
    "--val_set",
    required=False,
    help="percentage of training data to validation (default=1)",
    default=1,
    type=float)
parser.add_argument(
    "--gray",
...
parser.add_argument(
    "--video",
    required=False,
    help="use video capture device/video (default=0)",
    default=0)
parser.add_argument(
    "--save",
    required=False,
    help="save output image (default=False)",
    default=False,
    type=lambda s: s.lower() in true_cases)
...
if (Xv is not None):
    ...
    val_acc, val_acc_nc = classifier.my_evaluate(
        model, Xv, Yv, batch_size=128, criteria=args.eval_crit)
...
for i in np.arange(0, len_is):
    ...
    if (separate):
        print(
            "[INFO] Predicted: {0}, Confidence: {1:3.2f}, Second guess: {2} ({3}/{4})".
            format(guesses[0], confidence, guesses[1], i + 1, len_is))
        # NOTE: Expecting something like "./samples\\0.jpg"
        src = image_set[i]

        dest_file = src.split(os.sep)[-1]

        # NOTE: set manually by user
        dest_folder = "d:/datasets/dst_cameras/crops/training/"
        ...
        if (guesses[0] != true_label):
            ...
            if (show_image):
                print(
                    "Predicted: {0:2d}, Actual: {1:2d}, Confidence: {2:3.3f}, Second guess: {3:2d}".
                    format(
                        int(guesses[0]), np.argmax(label_set[i]), confidence,
                        int(guesses[1])))
                rgb = np.fliplr(image.reshape(-1, CHANNELS)).reshape(
                    image.shape)
                factor = 1
                rgb = cv2.resize(
                    rgb, (WIDTH * factor, HEIGHT * factor),
                    interpolation=cv2.INTER_CUBIC)
                # add predicted label to the image
                cv2.putText(rgb, str(true_label), (5, 20),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.75, 255, 2)
                cv2.putText(rgb, str(guesses[0]), (5, HEIGHT * factor - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.75, 255, 2)
                cv2.putText(rgb, str(guesses[1]),
                            (WIDTH * factor - 20, HEIGHT * factor - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.75, 255, 2)
                # shows image
...
print(
    colored(
        "[INFO] %d badly predicted images in a total of %d (Error rate %.4f)" %
        (bp, len_is, bp / len_is), "yellow"))
print(
    colored(
        "[INFO] %d well predicted images (confidence > %.2f) in a total of %d (Acc. %.4f)"
        % (wp, args.eval_crit, len_is, wp / len_is), "yellow"))
